...
simulate:
	java -cp "$(LIBJARFILE):build/classes/java/main" $(SERVERCLASS) -Djsettlers.allow.debug=Y -Djsettlers.startrobots=4 -Djsettlers.bots.percent3p=100 -Djsettlers.bots.start3p=1,$(BOTCLASS) -Djsettlers.bots.botgames.parallel=1 -Djsettlers.bots.botgames.wait_sec=1 -Djsettlers.bots.botgames.total=100 -Djsettlers.bots.botgames.shutdown=Y -Djsettlers.bots.cookie=$(COOKIE) > server.log
