...
build: $(FILES)
	javac -d "./build/classes/java/" -cp "lib/jsettlers2/build/libs/JSettlers-2.2.00.jar" $(FILES)

run:	build/
	java -cp "lib/jsettlers2/build/libs/JSettlers-2.2.00.jar:build/classes/java" -Djsettlers.debug.traffic=Y bot.NDRobotClient -c $(COOKIE)
